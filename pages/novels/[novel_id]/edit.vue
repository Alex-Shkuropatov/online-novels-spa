<template>
  <div class="container py-4">
    <h1>Edit Novel</h1>
    <p v-if="!novel">Loading...</p>
    <div v-else>
      <NovelImageUploader
          :novel-id="novelId"
          :image-url="novel.cover_image_url"
          @update:imageUrl="handleImageUpdate"
      />
...
await useAsyncData(`novel-${novelId.value}`, async () => {
  ...
  const { data, error } = await useFetch(`http://127.0.0.1:8000/novels/${novelId.value}`, {
    method: 'GET',
    headers: {
  ...
  if (error.value) {
    console.error('Failed to load novel:', error.value);
    throw createError({ statusCode: 500, message: 'Failed to load novel' });
  }

  // Set novel data into Pinia store
  novelStore.setNovel(data.value!);
});
...
async function generateMetadataFieldWithAi(fieldName: 'title' | 'description' | 'setting'): Promise<void> {
  ...
  try {
    const { data, error } = await useFetch(`http://127.0.0.1:8000/ai/novels/${novel.novel_id}/metadata`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: 'application/json',
    ...
    if (error.value) {
      throw new Error(`AI generation failed: ${error.value.message}`);
    }

    const generatedValue = data.value?.[fieldName];
    if (generatedValue) {
...
async function submitEditedNovel(): Promise<void> {
  ...
  try {
    const { error } = await useFetch(`http://127.0.0.1:8000/novels/${novel.novel_id}`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: 'application/json',
